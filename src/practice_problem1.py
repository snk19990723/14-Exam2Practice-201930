...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        # ---------------------------------------------------------------------
        contents_remain=self.volume-len(self.contents)
        s=''
        if len(self.contents+additional_contents)<=self.volume:
            self.contents=self.contents+additional_contents
            return s
        else :
            for k in range (contents_remain):
                self.contents=self.contents+additional_contents[k]
            for j in range (contents_remain,len(additional_contents)):
                s=s+additional_contents[j]
            return s
    ...
    def reset(self):
        ...
        self.contents = self.contents_start
